...
export default class LoadPDFComponent {
	...
	async init() {
		...
		const totalPage =
			pageInfo.totalPage <= this.MAXCOUNT ? pageInfo.totalPage : this.MAXCOUNT // this.totalPage
	...
	createCanvas(pageSize, pageNo) {
		...
		if (this.canvasDoms.length !== 0) {
			...
			canvas.style.webkitTransform = `translate3d(0, ${
				(pageNo - 1) * pageSize.height
			}px, 0)`
			return canvas
		}
	...
	getRenderScope(pageIndex) {
		...
		while (
			pagesToRender.length < COUNT &&
			pagesToRender.length < this.pages.length
		) {
			if (i > 0) {
				// console.log('i' ,  i - 1)
	...
	async renderPages(pageIndex) {
		// 可以直接把当前的加进去
		const { pagesToRender, min, max } = this.getRenderScope(pageIndex)
